...
#     ntuws_demographic_conflicts.csv     無解的矛盾（=真的被排除），每人每欄位一列
#     ntuws_demographic_conflicts_resolved.csv
#                                         被補救規則救回的矛盾（已進主表），
#                                         多 resolution / resolved_value 兩欄
#                                         （兩份都含完整作答軌跡，
#                                          見下方「矛盾清單怎麼看」）
#     ntuws_within_wave_conflicts.csv     同一波次內自我矛盾者的明細
#                                         （這些 memberId 整個排除）
#     ntuws_wave_participation_long.csv   去重後的標準化長表，供矛盾回溯
#
# -----------------------------------------------------------------------------
# 流程
#     STAGE 1  來源檔結構驗證     sheet 名稱、欄索引、題目文字比對
#     STAGE 2  讀取與值標準化     -> 長表
#     STAGE 3  同波重複去重       -> 六欄全同留一列；有互斥則整人排除
#     STAGE 4  參與軌跡彙整       -> 填答次數、首末波次
#     STAGE 5  矛盾判定           -> 依使用說明 §4 的優先序
#              + 矛盾補救           -> R3 波次多數決 / R1,R2 回頭比對 Welcome
#     STAGE 6  主表組裝           -> 不變欄位補值、可變欄位取最新
#     STAGE 7  輸出與摘要
#
# -----------------------------------------------------------------------------
...
#     MAJORITY_MIN_ANSWERS / MAJORITY_STRICT
#                       R3 多數決的門檻（在 SECTION 6.5）
...
#                          只是排序結果，不代表多數決答案
#   補救結果（decides 這一列是進主表還是被排除）
#     resolution           哪條規則救回的，只在 *_resolved.csv：
#                            R3_majority             波次嚴格多數
#                            R1_welcome_birth_year   出生年比對 Welcome
#                            R2_welcome_sex          性別比對 Welcome
#     resolved_value       規則判定採用的值，會寫進主表該欄
#                          （主表另有 sex_source / birth_year_source /
#                            father_ethnicity_source 與 conflict_resolved 可回查）
#
...
# =============================================================================
# SECTION 5.5 — 同波重複填答的去重與排除
#
# 同一個波次（sheet）裡，一個 memberId 照理只會有一筆填答。實際上有 420 組
# （406 人、875 列）違反這個前提。Phase1 已把它們獨立抓出來供人工檢視，
# 這裡則決定 Phase0 要怎麼用：
#
#   六個 demographic 欄位全部相同 -> 純粹重送同一份，只留第一列
#   任一欄位互斥                  -> 同一次問卷裡自己講兩套，資料不可信，
#                                    該 memberId 整個排除
#
# 為什麼比全部六欄（不只 FIXED 三欄）：
#   age / education / city 允許「跨波」隨時間變，但不允許「同一波之內」不同。
#   同一次填答裡年齡同時是 30 又是 45，那不是時間造成的，是資料有問題。
#
# 為什麼互斥就整個人排除而不是只刪該波：
#   與跨波矛盾的處理方式一致（all-or-nothing）。同一份問卷裡自我矛盾，
#   代表這個 ID 背後可能不是同一個人（實測有 ID 在同波填出兩組完全不同的
#   性別／出生年／學歷），其他波次的作答也就不可信。
#
# 實測沒有「一邊空白一邊有值」的情形 —— 所有差異都是真的互斥值，
# 因此不需要「空值可互補」的邏輯。
#
# 這一步在矛盾判定「之前」執行，所以：
#   - 去重後 n_waves 必然等於 n_sheets
#   - 跨波矛盾判定與 R3 多數決不再被同波重複的票數灌水
# =============================================================================

# 判斷同不同用這六欄（= FIXED + VARYING 全部）
DEDUP_FIELDS <- c(FIXED_FIELDS, VARYING_FIELDS)

# 回傳去重後的 long，以及因同波自我矛盾而該整個排除的 memberId 與明細。
dedup_within_wave <- function(long) {
  responses <- long %>% filter(sheet %in% RESPONSE_SHEETS)

  groups <- responses %>%
    group_by(memberId, sheet) %>%
    filter(n() > 1) %>%
    ungroup()

  if (!nrow(groups)) {
    say("  沒有同波重複填答")
    return(list(long = long, conflict_ids = character(), detail = NULL))
  }

  # 每個 (memberId, sheet) 組合：六欄是否全數一致
  # 逐欄數相異值：n_distinct 把 NA 當成一個值，所以整組皆 NA 會回 1（視為相同）
  status <- groups %>%
    group_by(memberId, sheet) %>%
    summarise(
      n_rows        = n(),
      rows_in_sheet = paste(row_in_sheet, collapse = "; "),
      across(all_of(DEDUP_FIELDS), n_distinct, .names = "nd_{.col}"),
      .groups = "drop"
    )

  # 哪幾欄不一致 —— 記下來方便回原檔查
  nd <- as.matrix(status[paste0("nd_", DEDUP_FIELDS)]) > 1
  status <- status %>%
    mutate(is_identical = !apply(nd, 1, any),
           diff_fields  = apply(nd, 1, function(v)
                                paste(DEDUP_FIELDS[v], collapse = "; "))) %>%
    select(-starts_with("nd_"))

  bad_ids <- unique(status$memberId[!status$is_identical])

  # 明細：互斥組的每一列原樣列出（六欄 + 列號），供人工回溯
  detail <- groups %>%
    semi_join(filter(status, !is_identical), by = c("memberId", "sheet")) %>%
    arrange(memberId, sheet, row_in_sheet) %>%
    select(memberId, sheet, row_in_sheet, all_of(DEDUP_FIELDS), education_raw) %>%
    left_join(select(status, memberId, sheet, n_rows, diff_fields),
              by = c("memberId", "sheet")) %>%
    relocate(n_rows, diff_fields, .after = sheet)

  # 去重：一致的組只留第一列；互斥的組整個人稍後會被移除，這裡不必特別處理
  kept <- responses %>%
    group_by(memberId, sheet) %>%
    slice_min(row_in_sheet, n = 1, with_ties = FALSE) %>%
    ungroup()

  out <- bind_rows(long %>% filter(sheet == "Welcome"), kept) %>%
    filter(!memberId %in% bad_ids) %>%
    arrange(memberId, sheet, row_in_sheet)

  n_ident <- sum(status$is_identical)
  say("  同波重複 ", nrow(status), " 組（", n_distinct(status$memberId), " 人 / ",
      sum(status$n_rows), " 列）")
  say("    六欄全同 -> 留一列：", n_ident, " 組（移除 ",
      sum(status$n_rows[status$is_identical]) - n_ident, " 列）")
  say("    有互斥   -> 整人排除：", nrow(status) - n_ident, " 組 / ",
      length(bad_ids), " 人")
  if (nrow(detail)) {
    tab <- sort(table(unlist(strsplit(
      unique(status$diff_fields[!status$is_identical]), "; "))), decreasing = TRUE)
    say("    互斥出現在：", paste(sprintf("%s(%d)", names(tab), tab), collapse = "  "))
  }
  say("  去重後 ", format(nrow(out), big.mark = ","), " 列 / ",
      format(n_distinct(out$memberId), big.mark = ","), " 人")

  # 不變條件：去重後每個 (memberId, sheet) 至多一列
  assert(!any(duplicated(
    out %>% filter(sheet != "Welcome") %>% select(memberId, sheet))),
    "去重後仍有同波重複")

  list(long = out, conflict_ids = bad_ids, detail = detail)
}
...
# =============================================================================
# SECTION 6.5 — 矛盾的補救規則（放寬排除）
#
# 原本只要任一欄位矛盾就整個人排除，代價太重：1,205 人（7.7%）出局，
# 其中很多只是單次手滑。這裡在「判定完矛盾之後」加三條補救規則，
# 能推定出唯一合理答案的就把該欄位判定為已解決，人留下來。
#
#   R3  波次多數決   該欄在波次間答過 >= MAJORITY_MIN_ANSWERS 次，
#                    且存在唯一的嚴格多數值（次數 > 一半）-> 採用該值
#   R1  出生年對 Welcome  波次值中有任一筆落在 Welcome ±BIRTH_YEAR_TOL
#                        -> 採用 Welcome 的年份
#   R2  性別對 Welcome    Welcome 的性別出現在波次答案中 -> 採用 Welcome 的值
#
# 優先序：R3 先，不成才回頭問 Welcome。
#   依據使用說明 §4「該波有詢問就優先使用該波回答」，Welcome 是備援基準，
#   不是與波次平起平坐的資料來源。波次自己能形成多數時，沒有理由讓
#   Welcome 的單一筆去推翻它（實測 sex 有 41 人兩者結論相反）。
#
# 為什麼 R3 要求答過 >= 2 次：
#   波次只答過 1 次時根本不存在「多數」，那是「單一筆未經佐證的答案
#   vs Welcome」，兩邊都可能錯。這種情形維持排除（birth_year 有 405 人
#   屬此類），不讓孤證推翻基準。
#
# 沒有任何一條規則能救的欄位，該 memberId 仍然整個人排除，行為與原本相同。
# =============================================================================

# R3 的門檻。改這兩個值就能收緊或放寬多數決。
MAJORITY_MIN_ANSWERS <- 2L    # 波次至少要答過幾次才談得上「多數」
MAJORITY_STRICT      <- TRUE  # TRUE = 需嚴格過半；FALSE = 唯一最多值即可

# --- 單一欄位的補救 ----------------------------------------------------------
# 回傳 resolution（NA = 救不了）與 resolved_value。
# vals 是該人該欄「參與判定」的波次答案（已排除 NA 與「不知道」）。
resolve_one <- function(field, vals, welcome_value) {
  n <- length(vals)

  # --- R3 波次多數決 ---
  if (n >= MAJORITY_MIN_ANSWERS) {
    tab  <- sort(table(vals), decreasing = TRUE)
    top  <- tab[1]
    tied <- length(tab) > 1 && tab[2] == top          # 並列最多 -> 不算多數
    enough <- if (MAJORITY_STRICT) top * 2 > n else TRUE
    if (!tied && enough) {
      return(list(resolution = "R3_majority", value = names(top)))
    }
  }

  if (is.na(welcome_value)) return(list(resolution = NA_character_, value = NA_character_))

  # --- R1 出生年比對 Welcome ---
  if (field == "birth_year") {
    if (any(abs(as.integer(vals) - as.integer(welcome_value)) <= BIRTH_YEAR_TOL)) {
      return(list(resolution = "R1_welcome_birth_year", value = welcome_value))
    }
  }

  # --- R2 性別比對 Welcome ---
  if (field == "sex" && welcome_value %in% vals) {
    return(list(resolution = "R2_welcome_sex", value = welcome_value))
  }

  list(resolution = NA_character_, value = NA_character_)
}

# --- 對所有矛盾欄位跑一遍 ----------------------------------------------------
# 輸出每個 (memberId, field) 一列：救得了的填 resolution/resolved_value，
# 救不了的兩欄皆 NA。
resolve_conflicts <- function(conflicts, wave_fixed, welcome_ref) {
  # Welcome 的對應值：長格式，方便按 field 取
  wel_long <- welcome_ref %>%
    rename_with(~ sub("^welcome_", "", .x), -memberId) %>%
    mutate(birth_year = as.character(birth_year)) %>%
    pivot_longer(-memberId, names_to = "field", values_to = "welcome_value")

  answers <- wave_fixed %>%
    semi_join(distinct(conflicts, memberId, field), by = c("memberId", "field")) %>%
    group_by(memberId, field) %>%
    summarise(vals = list(value), .groups = "drop") %>%
    left_join(wel_long, by = c("memberId", "field"))

  out <- answers %>%
    mutate(res = pmap(list(field, vals, welcome_value), resolve_one),
           resolution     = map_chr(res, "resolution"),
           resolved_value = map_chr(res, "value")) %>%
    select(memberId, field, resolution, resolved_value)

  n_res <- sum(!is.na(out$resolution))
  say("  補救規則：", nrow(out), " 筆欄位矛盾中解決 ", n_res,
      "（剩 ", nrow(out) - n_res, " 筆無解）")
  for (f in c("birth_year", "sex", "father_ethnicity")) {
    sub <- out %>% filter(field == f)
    if (!nrow(sub)) next
    tab <- table(ifelse(is.na(sub$resolution), "無解", sub$resolution))
    say("    ", f, "：", paste(sprintf("%s=%d", names(tab), tab), collapse = "  "))
  }
  out
}
...
adjudicate <- function(long, participation) {
  ...
  # --- 補救（SECTION 6.5）---------------------------------------------------
  # 判定不動，補救另外算。conflicts 帶上 resolution 後拆兩份：
  #   resolved   規則救得回來 -> 該欄位視為已解決，人進主表
  #   unresolved 救不回來     -> 該 memberId 仍整個人排除
  conflicts <- conflicts %>%
    left_join(resolve_conflicts(conflicts, wave_fixed, welcome_ref),
              by = c("memberId", "field")) %>%
    relocate(resolution, resolved_value, .after = field)

  # unresolved 的 resolution/resolved_value 依定義全是 NA，留著只會誤導，直接拿掉
  unresolved <- conflicts %>% filter(is.na(resolution)) %>%
    select(-resolution, -resolved_value)
  resolved   <- conflicts %>% filter(!is.na(resolution))

  # 排除是人層級的：任一欄位救不回來，該人所有波次都不進主表
  conflict_ids <- unique(unresolved$memberId)

  # 被救回的人也可能同時有其他欄位無解，那仍然要排除；
  # overrides 只餵給真正進主表的人（build_demographics 會再過濾一次）。
  say("  補救後：排除 ", format(length(conflict_ids), big.mark = ","), " 人",
      "（原 ", format(n_distinct(conflicts$memberId), big.mark = ","), " 人，",
      "救回 ", format(n_distinct(conflicts$memberId) - length(conflict_ids),
                      big.mark = ","), " 人）")

  assert(all(!is.na(resolved$resolved_value)), "已解決的欄位必須有 resolved_value")

  list(conflicts    = unresolved,
       resolved     = resolved,
       conflict_ids = conflict_ids,
       overrides    = resolved %>% select(memberId, field, resolution, resolved_value),
...
# --- 不變欄位 ----------------------------------------------------------------
# 走到這裡的人分兩種：
#   從頭就沒矛盾 -> 波次值至多一個相異值，first_non_na 即可
#   被 SECTION 6.5 救回 -> 波次值本來就互相矛盾，first_non_na 會挑到任意一個，
#                          所以最後要用 overrides 把補救規則判定的值蓋回去
resolve_fixed <- function(keep, welcome_ref, overrides) {
...
# 把補救規則判定的值蓋回去。
#
# 為什麼一定要蓋：這些人的波次答案彼此矛盾，上面的 coalesce/first_non_na
# 只會挑到「排序上碰巧第一個」的值。R3 救回的人尤其關鍵 ——
# birth_year 若不蓋，他們會拿到 Welcome 的年份，
# 也就是我們剛判定為與所有波次不符的那個值，等於白救。
#
# 順便把每欄實際的取值來源記進 *_source，事後可稽核是哪條規則決定的。
apply_overrides <- function(fixed, overrides) {
  wide <- overrides %>%
    select(memberId, field, resolution, resolved_value) %>%
    pivot_wider(names_from = field,
                values_from = c(resolution, resolved_value),
                names_glue = "{field}_{.value}")

  # 該欄位沒有任何人被救回時，pivot_wider 不會生出對應欄，先補齊
  for (f in FIXED_FIELDS) {
    for (suffix in c("resolution", "resolved_value")) {
      col <- paste0(f, "_", suffix)
      if (!col %in% names(wide)) wide[[col]] <- NA_character_
    }
  }

  fixed %>%
    left_join(wide, by = "memberId") %>%
    mutate(
      sex        = coalesce(sex_resolved_value, sex),
      sex_source = coalesce(sex_resolution, sex_source),

      father_ethnicity        = coalesce(father_ethnicity_resolved_value,
                                         father_ethnicity),
      father_ethnicity_source = coalesce(father_ethnicity_resolution,
                                         if_else(is.na(father_ethnicity),
                                                 NA_character_, "wave")),

      birth_year        = coalesce(as.integer(birth_year_resolved_value), birth_year),
      birth_year_source = coalesce(birth_year_resolution,
                                   if_else(is.na(birth_year), NA_character_,
                                           "welcome_or_wave_median"))
    ) %>%
    select(memberId, sex, sex_source, father_ethnicity, father_ethnicity_source,
           birth_year, birth_year_source)
}
...
build_demographics <- function(long, participation, conflict_ids, welcome_ref,
  ...
  # 有些人某欄被救回、另一欄仍無解，整個人還是排除；overrides 要跟著剔除
  overrides <- overrides %>% filter(!memberId %in% conflict_ids)
  ...
  out <- participation %>%
    ...
    left_join(resolve_fixed(keep, welcome_ref, overrides), by = "memberId") %>%
    ...
    # 這個人是靠補救規則進來的嗎？空白 = 從頭就沒矛盾
    left_join(overrides %>%
                arrange(memberId, field) %>%
                group_by(memberId) %>%
                summarise(conflict_resolved = paste(field, resolution,
                                                    sep = "=", collapse = "; "),
                          .groups = "drop"),
              by = "memberId") %>%
    select(memberId, n_waves, n_sheets,
           ...
           age, birth_year, birth_year_source, age_from_birth_year,
           father_ethnicity, father_ethnicity_source,
           education, education_raw, city,
           n_demo_fields, conflict_resolved) %>%
  ...
  assert(all(is.na(out$conflict_resolved) |
             out$memberId %in% overrides$memberId),
         "conflict_resolved 標記與 overrides 不一致")
  assert(all(!out$memberId[!is.na(out$conflict_resolved)] %in% conflict_ids),
         "被救回者不應同時在排除名單中")

  say("  靠補救規則進主表 ", format(sum(!is.na(out$conflict_resolved)),
                                    big.mark = ","), " 人")
...
print_summary <- function(long, demographics, conflicts, dedup_ids = character()) {
  # long 是去重後的，分母要加回同波自我矛盾被移除的人才是原始總數
  total <- n_distinct(long$memberId) + length(dedup_ids)
  n_cf  <- n_distinct(conflicts$memberId)

  say("\n", strrep("-", 72), "\n執行摘要\n", strrep("-", 72))
  say(sprintf("  原始 memberId : %s", format(total, big.mark = ",")))
  say(sprintf("  進入主表      : %s (%.1f%%)",
              format(nrow(demographics), big.mark = ","),
              100 * nrow(demographics) / total))
  say(sprintf("  同波自我矛盾  : %s (%.1f%%)",
              format(length(dedup_ids), big.mark = ","),
              100 * length(dedup_ids) / total))
  say(sprintf("  跨波矛盾排除  : %s (%.1f%%)",
...
say_stage(3, "同波重複填答的去重與排除")
dedup <- dedup_within_wave(long)
long  <- dedup$long

say_stage(4, "參與軌跡彙整")
participation <- build_participation(long)

say_stage(5, "矛盾判定（使用說明 §4）")
...
# 矛盾清單只留「補救規則救不回來、因此真的被排除」的欄位矛盾
write_csv_utf8(adj$conflicts, "ntuws_demographic_conflicts.csv")
# 被救回的矛盾另存一份：進了主表，但值是規則判出來的，需要時可回頭稽核
write_csv_utf8(adj$resolved,  "ntuws_demographic_conflicts_resolved.csv")
# 同波自我矛盾而被整個排除者的明細
if (!is.null(dedup$detail) && nrow(dedup$detail)) {
  write_csv_utf8(dedup$detail, "ntuws_within_wave_conflicts.csv")
}
# 長表：去重後、標準化的每一筆作答，矛盾需要回溯時用這張
write_csv_utf8(long %>% mutate(sheet = as.character(sheet)),
               "ntuws_wave_participation_long.csv")

print_summary(long, demographics, adj$conflicts, dedup$conflict_ids)
say("\n完成，耗時 ",
